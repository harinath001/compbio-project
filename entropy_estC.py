...
def entropy_estC(f):
    ...
    if not fmax:
        x = x[1:]
        histx = histx[1:]
        ent = -np.matmul(histx, np.array([y*math.log(y) for y in x]).T) + sum(histx)/(2*k)
        return ent
    ...
    if len(np.where(x > 0)[0]) == 0:
        ent = -np.matmul(sol2[0:szLPx].T , np.array([x*math.log(x) for x in xLP]))
    else:
        ent = - np.matmul(histx * (np.where(x>0)),     ((x*np.where(x>0)) * (np.array([math.log(y) for y in np.where(x>0)]))).T ) \
        + ( sum(histx[np.where(x>0)]) ) / (2*k - np.matmul(sol2[1:szLPx].T, (xLP*np.array([math.log(y) for y in x])).T  ) )
...
inp = makeFinger([5, 6, 9, 20, 19, 12, 12, 2, 12, 18])
print(inp)
print(unseen(inp))
print(entropy_estC(inp))
