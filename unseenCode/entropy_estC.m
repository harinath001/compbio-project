...
if min(size(fmax))==0
    x=x(2:end);
    histx=histx(2:end);
    ent = -histx*(x.*log(x))' +(sum(histx))/(2*k);
    return;
end
...
if min(size(find(x>0)))==0
    ent=-(sol2(1:szLPx)')*(xLP.*log(xLP))';
else
    ent=-histx(find(x>0))*(x(find(x>0)).*log(x(find(x>0))))'+(sum(histx(find(x>0))))/(2*k)-(sol2(1:szLPx)')*(xLP.*log(xLP))';
end
